...
const desired = { dshVersion: '0.1.1-rc.2', plugins: [], skills: [], runtimes: [], defaultPermission: 'workspace-write', modelRoute: 'local-gateway-required' };
...
test('project.open persists verified plugin sync status and reconcile returns it without owning Session storage', async () => {
  ...
  const desiredWithPlugin = {
    ...desired,
    plugins: [{ id: 'remote-plugin', version: '1.0.0', placement: 'remote', source: { registry: 'trusted', artifact: 'remote-plugin.tgz' }, sha256: 'a'.repeat(64), compatibility: { dsh: { min: '0.1.0-rc.6', max: '0.1.1-rc.2' }, api: { min: '1.0', max: '1.0' } }, requiredBy: ['project:remote'] }],
  };
