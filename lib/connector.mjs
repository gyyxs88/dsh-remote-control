...
export class RemoteControlConnector {
  constructor({ transport, sourceHostId = `local-${randomUUID()}`, sourceSessionId = `session-${randomUUID()}`, cache = new MemoryConnectorCache(), capabilities = DEFAULT_CLIENT_CAPABILITIES, desiredStateSynchronizer = null, runtimeSynchronizer = null } = {}) {
  ...
  async openProject({ absolutePath, desiredState = { dshVersion: '0.1.0-rc.6', plugins: [], skills: [], runtimes: [], defaultPermission: 'workspace-write', modelRoute: 'local-gateway-required' }, displayName, schedule, idempotencyKey } = {}) {
    ...
    const body = { absolutePath, desiredState: normalizedDesiredState, pluginSync, runtimeSync, ...(displayName ? { displayName } : {}), ...(schedule ? { schedule } : {}) };
  ...
  async runtimeAuthChallenge(requirement, output = null) {
    this.#assertConnected();
    const response = await this.#request({ type: 'runtime.auth.challenge', requirement, ...(typeof output === 'string' ? { output: output.slice(0, 16_384) } : {}), sourceHostId: this.sourceHostId, sourceSessionId: this.sourceSessionId, targetHostId: this.connection.host.hostId });
    this.#acceptRevision(response);
    return response;
  }
