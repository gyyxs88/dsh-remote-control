...
import { compareSemver, parseSemver, remoteRequirements, requirementIsCompatible, validatePluginRequirement, validateSkillRequirement } from './desired-state.mjs';
...
function validateCatalogEntry(entry) {
  if (!entry || typeof entry !== 'object' || !['plugin', 'skill'].includes(entry.kind)) invalid('trusted artifact registry entry is invalid');
  ...
  if (typeof entry.packageName !== 'string' || !PACKAGE_NAME.test(entry.packageName)) invalid('trusted packageName is invalid');
...
export class TrustedArtifactRegistry {
  ...
  async resolve(requirement, { dshVersion, apiVersion } = {}) {
    const normalized = requirement.kind === 'skill' ? validateSkillRequirement(requirement) : validatePluginRequirement(requirement);
    ...
    if (entry.sha256 !== normalized.sha256 || entry.version !== normalized.version || entry.id !== normalized.id) throw new DshRemoteError('desired state digest or version does not match the trusted allowlist', { code: 'PLUGIN_TRUST_MISMATCH', details: { id: normalized.id, version: normalized.version } });
  ...
  async resolveDesiredState(desiredState) {
    const results = [];
    for (const { kind, requirement } of remoteRequirements(desiredState)) results.push(await this.resolve({ kind, ...requirement }, desiredState));
    return results;
  }
}
