...
async function currentVersion({ root, kind, id }) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') return { status: 'missing', current };
    throw error;
  }
}
...
export async function inspectRemoteArtifact({ kind, id, installRoot } = {}) {
  ...
  if (!manifest || manifest.kind !== kind || manifest.id !== id || !SEMVER.test(manifest.version ?? '') || !SHA256.test(manifest.sha256 ?? '') || !Number.isSafeInteger(manifest.size) || manifest.size <= 0 || typeof manifest.packageName !== 'string' || !/^\d+\.\d+$/u.test(manifest.protocolVersion ?? '') || manifest.target !== 'linux-x86_64') fail('installed artifact manifest is invalid', 'PLUGIN_MANIFEST_INVALID');
...
export async function installRemoteArtifact({ artifactPath, kind, id, version, expectedSha256, installRoot, packageName, protocolVersion = '1.0', target = 'linux-x86_64', linkType = 'dir' } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (!currentInfo.isSymbolicLink()) fail('current artifact pointer is not a symlink', 'PLUGIN_POINTER_INVALID');
      const currentTarget = await realpath(current);
      if (!isWithinRoot(await realpath(componentRoot), currentTarget)) fail('current artifact pointer escapes install root', 'PLUGIN_INSTALL_ROOT_ESCAPE');
    } catch (error) {
      if (error?.code !== 'ENOENT') throw error;
    }
    nextLink = join(componentRoot, `.current-${process.pid}-${Math.random().toString(16).slice(2)}`);
