...
export function validateSshHostKeyPolicy(policy = {}) {
  ...
  if (policy.expectedFingerprint && !/^SHA256:[A-Za-z0-9_-]+$/u.test(policy.expectedFingerprint)) throw new DshRemoteError('SSH host-key fingerprint must be an OpenSSH SHA256 fingerprint', { code: 'SSH_HOST_KEY_PIN_INVALID' });
...
export function verifyPinnedHostKeySync(policy) {
  ...
  if (pin.expectedFingerprint) {
    const fingerprint = `SHA256:${createHash('sha256').update(Buffer.from(match.data, 'base64')).digest('base64url')}`;
  ...
  return { ...pin, keyType: match.type, fingerprint: `SHA256:${createHash('sha256').update(Buffer.from(match.data, 'base64')).digest('base64url')}` };
}
