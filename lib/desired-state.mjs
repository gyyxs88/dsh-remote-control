...
export const API_VERSION_PATTERN = /^(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)$/u;
...
export const RUNTIME_ID_PATTERN = /^(?:codex|claude-code|grok-build|acp\/[A-Za-z0-9][A-Za-z0-9._-]{0,63})$/u;
...
export function validateRuntimeRequirement(input) {
  ...
  const id = requiredString(input.id, 'runtime.id', RUNTIME_ID_PATTERN);
...
export function validateDesiredState(input, { allowRuntimes = true } = {}) {
  ...
  const apiVersion = requiredString(input.apiVersion ?? DESIRED_STATE_API_VERSION, 'desiredState.apiVersion', API_VERSION_PATTERN);
  ...
  const seen = new Set();
  for (const [kind, requirements] of [['plugin', plugins], ['skill', skills], ['runtime', runtimes]]) {
    for (const requirement of requirements) {
      const key = `${kind}\u0000${requirement.id}\u0000${requirement.version}\u0000${requirement.placement}`;
      if (seen.has(key)) fail('desiredState contains a duplicate requirement', { kind, id: requirement.id, version: requirement.version });
      seen.add(key);
    }
  }
  return {
...
export function requirementIsCompatible(requirement, { dshVersion, apiVersion } = {}) {
  ...
  const api = requiredString(apiVersion, 'current apiVersion', API_VERSION_PATTERN);
...
function parseApiVersion(value, field) {
  requiredString(value, field, API_VERSION_PATTERN);
