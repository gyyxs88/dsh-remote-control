...
- `dsh.control` manifest 固定 `dsh-remote-project 0.1.1` 的完整文件 SHA-256；真实 npm pack/install 后再次读取 Skill 并核对摘要。
...
- artifact provider 对当前受信本包、同 profile 的 `dsh-session-control` 使用 `npm pack --ignore-scripts`，对精确 DSH lock 生成只含两个 manifest 的 recipe；recipe 必须把每个非可选 peer 解析到锁文件顶层、所有根依赖均为精确版本，并按包名和版本固定五项 lifecycle 许可；项目输入不能成为 artifact trust root。
...
- 先完成 rc.6 基线，随后原子升级并启动 DSH `0.1.0-rc.8`、Remote Host `0.2.1` 与 `dsh-session-control 0.6.5`，均为非 root 用户目录和 user service；旧版本目录保留。
- 正式 rc.8 项目链路复用 Workspace `12d653ff-65db-467c-8bb1-1eddaf835c2b`，创建 Session `session-15c346c0-80a3-42a3-8c67-7bcdcb92e0c7`，权限为 `workspace-write`。
