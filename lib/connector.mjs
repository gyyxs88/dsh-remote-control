...
import { remoteRequirements, validateDesiredState } from './desired-state.mjs';
...
export class RemoteControlConnector {
  constructor({ transport, sourceHostId = `local-${randomUUID()}`, sourceSessionId = `session-${randomUUID()}`, cache = new MemoryConnectorCache(), capabilities = DEFAULT_CLIENT_CAPABILITIES, desiredStateSynchronizer = null } = {}) {
  ...
  async openProject({ absolutePath, desiredState = { dshVersion: '0.1.0-rc.6', plugins: [], skills: [], runtimes: [], defaultPermission: 'workspace-write', modelRoute: 'local-gateway-required' }, displayName, schedule, idempotencyKey } = {}) {
    ...
    const body = { absolutePath, desiredState: normalizedDesiredState, pluginSync, ...(displayName ? { displayName } : {}), ...(schedule ? { schedule } : {}) };
