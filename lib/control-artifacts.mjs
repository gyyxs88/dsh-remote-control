...
async function buildDshRecipe(sourceRoot, outputDir, version, { tarPath = 'tar', spawnImpl = spawn } = {}) {
  ...
  if (packageJson.private !== true || dependencies.length !== 1 || dependencies[0][0] !== '@deepseek-ai/dsh' || dependencies[0][1] !== version) throw new DshRemoteError('DSH recipe source must contain only the exact DSH dependency', { code: 'CONTROL_DSH_RECIPE_INVALID' });
  const lockPath = path.join(sourceRoot, 'package-lock.json');
  const packageLock = JSON.parse(await readFile(lockPath, 'utf8'));
  if (packageLock.lockfileVersion !== 3 || packageLock.packages?.['']?.dependencies?.['@deepseek-ai/dsh'] !== version || packageLock.packages?.['node_modules/@deepseek-ai/dsh']?.version !== version) throw new DshRemoteError('DSH recipe lock does not bind the configured DSH version', { code: 'CONTROL_DSH_RECIPE_INVALID' });
