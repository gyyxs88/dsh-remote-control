...
const SAFE_REMOTE_UPLOAD_PATH = /^\/[A-Za-z0-9._/@+-]+$/u;
const SAFE_REMOTE_HOST_ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u;
const ALLOWED_EXTRA_FLAGS = new Set(['-N', '-T', '-o', '-R', '-L']);
  ...
  for (let index = 0; index < extraArgs.length; index += 1) {
    ...
    if (value.startsWith('-')) {
      ...
      if (value === '-o') {
        ...
        if (!/^127\.0\.0\.1:\d+:127\.0\.0\.1:\d+$/u.test(forward ?? '')) throw new DshRemoteError('SSH forward is not loopback-only', { code: 'SSH_TUNNEL_BIND_INVALID' });
      }
...
function buildRemoteHostOptionArgs(options) {
  if (!options || typeof options !== 'object' || Array.isArray(options)) throw new DshRemoteError('SSH stdio bridge requires explicit Remote Host options', { code: 'SSH_REMOTE_HOST_OPTIONS_REQUIRED' });
  const { sessionControlSocket, sessionControlModule, runtimeManagerSocket, runtimeManagerTokenFile, runtimeRoot, allowedRoot, hostId } = options;
  if (Boolean(sessionControlSocket) === Boolean(sessionControlModule)) throw new DshRemoteError('SSH stdio bridge requires exactly one Session Control socket or module', { code: 'SSH_SESSION_CONTROL_REQUIRED' });
  const result = [];
  const addPath = (flag, value, field) => {
    validateSafePosixPath(value, { field, allowHome: false });
    result.push(flag, value);
  };
  if (sessionControlSocket) addPath('--session-control-socket', sessionControlSocket, 'sessionControlSocket');
  if (sessionControlModule) addPath('--session-control-module', sessionControlModule, 'sessionControlModule');
  if (Boolean(runtimeManagerSocket) !== Boolean(runtimeManagerTokenFile)) throw new DshRemoteError('Runtime Manager socket and token file must be configured together', { code: 'SSH_RUNTIME_MANAGER_CONFIG_INVALID' });
  if (runtimeManagerSocket) {
    if (!SAFE_REMOTE_HOST_ID.test(hostId ?? '')) throw new DshRemoteError('Runtime Manager requires an explicit safe Host id', { code: 'SSH_REMOTE_HOST_ID_INVALID' });
    addPath('--runtime-manager-socket', runtimeManagerSocket, 'runtimeManagerSocket');
    addPath('--runtime-manager-token-file', runtimeManagerTokenFile, 'runtimeManagerTokenFile');
  }
  if (runtimeRoot) addPath('--runtime-root', runtimeRoot, 'runtimeRoot');
  if (allowedRoot) addPath('--allowed-root', allowedRoot, 'allowedRoot');
  if (hostId) {
    if (!SAFE_REMOTE_HOST_ID.test(hostId)) throw new DshRemoteError('Remote Host id is unsafe', { code: 'SSH_REMOTE_HOST_ID_INVALID' });
    result.push('--host-id', hostId);
  }
  return result;
}

export function buildStdioBridgeArgs({ policy, host, command = 'dsh-remote-host', dataDir, remoteHostOptions, extraArgs = [] } = {}) {
  ...
  const optionArgs = buildRemoteHostOptionArgs(remoteHostOptions);
  return buildStrictSshArgs({ policy, host, remoteCommand: [command, 'bridge', '--stdio', '--data-dir', dataDir, ...optionArgs], extraArgs });
}
...
export class SshCommandTransport {
  ...
  upload(localPath, remotePath) {
    ...
    if (!SAFE_REMOTE_UPLOAD_PATH.test(remotePath)) throw new DshRemoteError('remote upload path is not portable across OpenSSH SFTP/SCP modes', { code: 'SSH_UPLOAD_PATH_INVALID' });
    const args = buildStrictSshArgs({ policy: this.policy, host: this.host }).slice(0, -1);
    args.push(localPath, `${this.host}:${remotePath}`);
...
export function buildLocalTunnelArgs({ policy, host, localPort, remotePort, bindAddress = '127.0.0.1', remoteHost = '127.0.0.1', extraArgs = [] } = {}) {
  if (!Number.isInteger(localPort) || localPort < 1 || localPort > 65535 || !Number.isInteger(remotePort) || remotePort < 1 || remotePort > 65535) throw new DshRemoteError('local tunnel ports must be valid TCP ports', { code: 'SSH_TUNNEL_PORT_INVALID' });
  if (bindAddress !== '127.0.0.1' || remoteHost !== '127.0.0.1') throw new DshRemoteError('local tunnel must bind loopback on both sides', { code: 'SSH_TUNNEL_BIND_INVALID' });
  return buildStrictSshArgs({ policy, host, extraArgs: ['-N', '-T', '-o', 'ExitOnForwardFailure=yes', '-L', `${bindAddress}:${localPort}:${remoteHost}:${remotePort}`, ...extraArgs] });
}

export class SshLocalTunnel {
  constructor({ sshPath = 'ssh', policy, host, localPort, remotePort, spawnImpl = spawn } = {}) {
    this.sshPath = sshPath; this.policy = policy; this.host = host; this.localPort = localPort; this.remotePort = remotePort; this.spawnImpl = spawnImpl; this.child = null;
  }

  start() {
    if (this.child) return this.child;
    const args = buildLocalTunnelArgs({ policy: this.policy, host: this.host, localPort: this.localPort, remotePort: this.remotePort });
    const child = this.spawnImpl(this.sshPath, args, { stdio: ['ignore', 'ignore', 'pipe'], windowsHide: true });
    this.child = child;
...
export class SshStdioBridge {
  constructor({ sshPath = 'ssh', policy, host, command, dataDir, remoteHostOptions, spawnImpl = spawn, requestTimeoutMs = 30_000, maxFrameBytes = 256 * 1024, maxPending = 64 } = {}) {
    ...
    this.sshPath = sshPath; this.policy = policy; this.host = host; this.command = command; this.dataDir = dataDir; this.remoteHostOptions = remoteHostOptions; this.spawnImpl = spawnImpl; this.requestTimeoutMs = requestTimeoutMs; this.maxFrameBytes = maxFrameBytes; this.maxPending = maxPending;
    this.child = null; this.pending = new Map(); this.buffer = ''; this.nextRequestId = 1; this.protocolErrors = 0;
  }

  async start() {
    if (this.child) return;
    const args = buildStdioBridgeArgs({ policy: this.policy, host: this.host, command: this.command, dataDir: this.dataDir, remoteHostOptions: this.remoteHostOptions });
