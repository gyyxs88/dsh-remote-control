...
test('DSH recipe requires an exact top-level closure for every non-optional peer', () => {
  const packageJson = { private: true, dependencies: { '@deepseek-ai/dsh': '0.1.1-rc.2', '@deepseek-ai/required-peer': '0.1.1-rc.2' } };
  const packageLock = { lockfileVersion: 3, packages: {
    '': { dependencies: { ...packageJson.dependencies } },
    'node_modules/@deepseek-ai/dsh': { version: '0.1.1-rc.2', peerDependencies: { '@deepseek-ai/required-peer': '^0.1.1-rc.2' } },
    'node_modules/@deepseek-ai/required-peer': { version: '0.1.1-rc.2' },
  } };
  assert.doesNotThrow(() => validateDshRecipeLock(packageJson, packageLock, '0.1.1-rc.2'));
  const missing = structuredClone(packageJson);
  delete missing.dependencies['@deepseek-ai/required-peer'];
  assert.throws(() => validateDshRecipeLock(missing, packageLock, '0.1.1-rc.2'), (error) => error.code === 'CONTROL_DSH_RECIPE_INVALID' || error.code === 'CONTROL_DSH_RECIPE_PEER_CLOSURE_INVALID');
});
...
test('control artifact provider packs installed trusted sources without lifecycle scripts', async () => {
  ...
  assert.equal(artifacts.remoteHost.version, '0.2.2');
  assert.equal(artifacts.sessionControl.version, '0.6.6');
  assert.equal(artifacts.sessionControl.pluginRequirement.sha256, artifacts.sessionControl.sha256);
  assert.equal(artifacts.sessionControl.skillRequirement.bundledWith.pluginVersion, '0.6.6');
