...
export function validateOperationEnvelope(message) {
  ...
  if (!message.permissionSnapshot?.preset || !message.permissionSnapshot?.expiresAt) {
    throw new ProtocolError('operation permission snapshot is required');
  }
  return message;
}
...
export function validateProjectOpenBody(body) {
  ...
  if (normalizedPath !== absolutePath && absolutePath.includes('..')) {
