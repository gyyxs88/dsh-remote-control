...
function validateInputs({ kind, id, version, expectedSha256, installRoot, packageName }) {
  if (!['plugin', 'skill'].includes(kind) || !SAFE_ID.test(id ?? '') || !SEMVER.test(version ?? '') || !SHA256.test(expectedSha256 ?? '') || !PACKAGE_NAME.test(packageName ?? '')) fail('remote artifact installer inputs are invalid', 'PLUGIN_INSTALL_INPUT_INVALID');
  ...
  return { kind, id, version, expectedSha256, installRoot: resolve(installRoot), packageName };
}
...
function validatePackage(packageJson, { kind, id, version, packageName }) {
...
async function currentVersion({ root, kind, id }) {
  const componentRoot = join(root, kindDirectory(kind), id);
...
function validateManifest(manifest, { kind, id, expected = null } = {}) {
  ...
  if (expected) {
    if (expected.status !== 'installed' || manifest.version !== expected.version || manifest.sha256 !== expected.sha256 || manifest.size !== expected.size || manifest.packageName !== expected.packageName || manifest.protocolVersion !== expected.protocolVersion || manifest.target !== expected.target) fail('rollback target does not match the installed manifest', 'PLUGIN_ROLLBACK_TARGET_MISMATCH');
  }
  return manifest;
}
...
async function validateInstalledVersion({ root, kind, id, target }) {
  ...
  validatePackage(await readPackageJson(packageRoot), { kind, id, version: manifest.version, packageName: manifest.packageName });
  if (kind === 'skill') {
...
export async function inspectRemoteArtifact({ kind, id, installRoot } = {}) {
  if (!['plugin', 'skill'].includes(kind) || !SAFE_ID.test(id ?? '') || typeof installRoot !== 'string') fail('artifact status inputs are invalid', 'PLUGIN_STATUS_INPUT_INVALID');
  const root = resolve(installRoot);
  await canonicalAncestor(root);
  await canonicalAncestor(join(root, kindDirectory(kind), id));
  ...
  validatePackage(await readPackageJson(current.target), { kind, id, version: manifest.version, packageName: manifest.packageName });
  if (kind === 'skill') {
  ...
  return { status: 'installed', kind, id, version: manifest.version, sha256: manifest.sha256, size: manifest.size, packageName: manifest.packageName, target: manifest.target, protocolVersion: manifest.protocolVersion };
}

export async function installRemoteArtifact({ artifactPath, kind, id, version, expectedSha256, installRoot, packageName, protocolVersion = '1.0', target = 'linux-x86_64', linkType = 'dir' } = {}) {
  const inputs = validateInputs({ kind, id, version, expectedSha256, installRoot, packageName });
  ...
  try {
    ...
    if (existingVersion) {
      ...
      if (existingManifest.kind !== kind || existingManifest.id !== id || existingManifest.version !== version || existingManifest.sha256 !== expectedSha256 || existingManifest.size !== info.size || existingManifest.packageName !== packageName) fail('same version is already installed with a different artifact', 'PLUGIN_VERSION_CONFLICT');
      ...
      await writeFile(join(tempRoot, 'manifest.json'), `${JSON.stringify({ kind, id, version, packageName, protocolVersion, target, sha256: expectedSha256, size: info.size }, null, 2)}\n`, { mode: 0o600 });
      await rename(tempRoot, versionRoot);
      moved = true;
    }
    await switchCurrent({ componentRoot, version, linkType });
    return { status: moved ? 'installed' : 'already-installed', kind, id, version, sha256: expectedSha256, size: info.size, packageName, target, protocolVersion };
  } finally {
...
export async function rollbackRemoteArtifact({ kind, id, installRoot, target, linkType = 'dir' } = {}) {
  if (!['dir', 'junction'].includes(linkType)) fail('remote artifact pointer type is invalid', 'PLUGIN_POINTER_INVALID');
  if (!['plugin', 'skill'].includes(kind) || !SAFE_ID.test(id ?? '') || typeof installRoot !== 'string') fail('rollback inputs are invalid', 'PLUGIN_ROLLBACK_TARGET_INVALID');
  const root = resolve(installRoot);
  const componentRoot = join(root, kindDirectory(kind), id);
  const current = join(componentRoot, 'current');
  await canonicalAncestor(root);
  ...
  if (target.status !== 'installed' || !SEMVER.test(target.version ?? '') || !SHA256.test(target.sha256 ?? '') || !Number.isSafeInteger(target.size) || target.size <= 0 || !PACKAGE_NAME.test(target.packageName ?? '') || target.target !== 'linux-x86_64' || !/^\d+\.\d+$/u.test(target.protocolVersion ?? '')) fail('rollback target receipt is invalid', 'PLUGIN_ROLLBACK_TARGET_INVALID');
