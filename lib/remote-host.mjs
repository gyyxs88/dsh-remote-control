...
export class RemoteHostDaemon {
  ...
  async #openProject(message) {
    ...
    try {
      ...
      const state = ['TRANSPORT_ERROR', 'UNKNOWN_TERMINAL_STATE', 'SESSION_CONTROL_UNAVAILABLE', 'SESSION_CONTROL_FRAME_INVALID', 'TIMEOUT', 'RUNTIME_AUTH_RESERVATION_INVALID', 'RUNTIME_AUTH_RESERVATION_CONFLICT', 'RUNTIME_AUTH_RESERVATION_UNSUPPORTED'].includes(error?.code) ? 'needs-attention' : 'failed';
  ...
  #canRetryUnknownProjectOpen(operation) {
    return operation?.type === 'project.open'
      && operation.state === 'needs-attention'
      && ['TRANSPORT_ERROR', 'UNKNOWN_TERMINAL_STATE', 'SESSION_CONTROL_UNAVAILABLE', 'SESSION_CONTROL_FRAME_INVALID', 'TIMEOUT', 'HOST_RESTART_INTERRUPTED'].includes(operation.error?.code);
  }
